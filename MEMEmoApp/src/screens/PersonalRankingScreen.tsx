...
const PersonalRankingScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const sessions: SessionDTO[] = await Backend.getPlayerSessions(userId);

      const uniqueUserId = Array.from(new Set(sessions.map((s) => s.userId)));

      const users = await Promise.all(
        uniqueUserId.map(async (id) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>🏆 Personal Ranking</Text>
      <FlatList
        data={ranking}
